...
tm
```
This will start an interactive session where you can run commands. The supported list of commands are: `show`, `add`, `addt`, `update`, `delete`, `queue`, `start`, `startt`, `check`, `checkt`, `save`, `today` and `quit`. The supported list of bulk commands are: `delete many`.

At this point you can freely delete the source file. To quickly display tasks without starting a session you can run
```bash
tm show
```
Similar quick commands are supported for `add`, `addt`, `queue`, `start`, `startt`, `check`, `checkt`, and `today`.
